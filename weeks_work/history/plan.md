...
| **4. lane_detect_hailo.py + 빌드 + 검증** | ✅ 완료 | DFL 후처리 + HEF 메타 기반 입력 크기 자동 감지 + 4채널 안전 가드 |
| **5. FPS 검증 (실주행)** | ✅ 완료 | **30 FPS 안정 / NPU 4.2ms / 콜백 33ms** (카메라 30Hz 병목, NPU 등가 236 FPS) |
| **6. 실세계 튜닝** | ⏸ 대기 | 카메라 오프셋/APF |
| **7. 통합 시나리오** | ⏸ 대기 | |
| **8. 안정성 보완** | ⏸ 대기 | healthcheck, 로깅, FastDDS XML |
| **9. 시연 + 보고서** | ⏸ 대기 | |

### Phase 1~5 실측치 (NCNN baseline 대비)

| 지표 | NCNN baseline | **Hailo-8L (HEF, 현재)** | 향상 |
|------|---------------|--------------------------|------|
...
| NPU 추론 시간 | 27~33 ms (CPU) | **4.2 ms** | **7×** |
| **실주행 FPS** | 13~15 | **29.3~30.0** (카메라 30Hz 병목) | **2×** |
| 콜백 전체 | 66~77 ms | **33 ms** | 2× |
| 등가 FPS (NPU 단독) | — | **236 (벤치마크 217)** | — |
