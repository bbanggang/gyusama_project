...
class TurtleBot3SceneCfg(InteractiveSceneCfg):
    ...
    # rough_plane USD 지형 (RayCaster가 메시 감지 가능)
    ground = AssetBaseCfg(
...
def run_simulation(sim: SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        # ── 휠 속도 명령 ──────────────────────────────────────────────────────
        # 전진: 두 바퀴 같은 방향으로 회전 (오른쪽 바퀴 축 반전 적용)
        if count % 500 < 300:
            wheel_vel = torch.tensor([[2.0, -2.0]], device=sim.device)  # 전진
        else:
            wheel_vel = torch.tensor([[2.0, -2.0]], device=sim.device)  # 계속 전진
