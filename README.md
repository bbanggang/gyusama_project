...
총 14주 과정으로 진행되며, 크게 5단계로 구분된다.

| 단계 | 주차 | 내용 | 상태 |
|------|------|------|------|
| **1단계 — DevOps** | 1~3주차 | Docker 환경 구축, 멀티 아키텍처 빌드, 원격 배포 | ✅ 완료 |
| **2단계 — 시뮬레이션** | 4~6주차 | URDF → USD 변환, 가상 센서 연동, 합성 데이터 생성 | ✅ 완료 |
| **3단계 — AI 모델** | 7~9주차 | YOLOv8 학습, ONNX 양자화, APF 장애물 회피 통합 | ✅ 완료 |
| **4단계 — 실물 배포** | 10~12주차 | RPi5 배포, NPU 장착, 실물 주행 검증 | 🔄 진행 중 |
| **5단계 — 최종 시연** | 13~14주차 | Docker Hub 파이프라인 완성, 최종 주행 시연 | ⏳ 예정 |

### 현재 진행 상황 (2026-05-25 기준)

| 주차 | 내용 | 상태 |
|------|------|------|
| W1~W9 | DevOps · Isaac Sim · AI 모델 · APF 구현 | ✅ 완료 |
| W10 | RPi5 실물 배포, LiDAR 연동, 장애물 감지 확인 | ✅ 완료 |
| W11 | NPU 장착, 추론 FPS 개선, 동시 주행 검증 | ⏳ 예정 |
| W12 | APF 파라미터 튜닝, 실물 주행 완성 | ⏳ 예정 |
| W13 | Docker Hub 배포 전환, Sim-to-Real 분석 | ⏳ 예정 |
| W14 | 최종 시연, 보고서 제출 | ⏳ 예정 |
...
- 차선 추종 트랙 완주율 **90% 이상**
- 장애물 회피 성공률 **80% 이상**
- RPi5 실시간 추론 **10 FPS 이상** (NPU 장착 후)
...
### 13주차 (5/27) — Docker Hub 배포 파이프라인 전환 및 Sim-to-Real 분석

#### 이번 주 목표
volume mount 방식에서 Docker Hub 이미지 배포 방식(Direction A)으로 전환하여 RPi5에서 `pull` 만으로 실행 가능한 배포 파이프라인을 완성하고, Sim-to-Real 갭을 분석·문서화한다.

#### 주요 구현

**13-1. Docker Hub 배포 파이프라인 (Direction A)**
```bash
# PC에서 ARM64 이미지 빌드 후 Docker Hub push
docker buildx build \
...
- `Dockerfile.rpi5`: `COPY models/inference_node/`, `COPY config/` 추가
- `docker-compose.yml`: volumes 제거, `image: bbanggang/gyusama-rpi5:latest` 적용

**13-2. Sim-to-Real 갭 분석**

| 항목 | Isaac Sim | RPi5 실물 | 차이 원인 |
|------|-----------|-----------|-----------|
| 추론 FPS | ~30 FPS | 10+ FPS (NPU) | 하드웨어 차이 |
| LiDAR Hz | 10 Hz | 9.4 Hz | 거의 동일 |
| 차선 검출 신뢰도 | 0.7~0.9 | 측정값 | 조명 차이 |
| APF `k_rep` | 0.8 | 재튜닝 값 | 실물 제동거리 |
| APF `influence` | 0.65 | 재튜닝 값 | 실물 LiDAR 노이즈 |

#### 완료 기준
- [ ] Docker Hub 이미지 push (`bbanggang/gyusama-rpi5:latest`)
- [ ] RPi5 `docker compose pull && up` 단독 실행 확인
- [ ] Sim-to-Real 갭 분석 문서화 완료

> 상세 내용: [`weeks_work/week13/README.md`](weeks_work/week13/README.md)

---

### 14주차 (6/3) — 최종 주행 시연 및 보고서 제출

#### 이번 주 목표
최종 시연 시나리오로 실물 트랙에서 차선 추종 + 장애물 회피 통합 주행을 완수하고, 프로젝트 보고서를 제출한다.
...
> 상세 내용: [`weeks_work/week14/README.md`](weeks_work/week14/README.md)
